...
export const getTydomDeviceData = async <T extends TydomEndpointData = TydomEndpointData>(
  ...
  const promise = client.get<TydomEndpointDataResponse>(uri).then((res) => {
    if (res.error > 0) {
      debug(
        `Received non-zero error=${chalkNumber(res.error)} while querying uri=${chalkString(uri)}`,
      );
      if (res.error > 10) {
        debug(
          `Accessory with uri=${chalkString(uri)} seems unreacheable (error=${chalkNumber(res.error)}).`,
        );
